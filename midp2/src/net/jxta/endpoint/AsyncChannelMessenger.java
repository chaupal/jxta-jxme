...
public abstract class AsyncChannelMessenger extends ChannelMessenger {
     ...
     * Log4J Category
     */
    private static final Logger LOG = Logger.getInstance(AsyncChannelMessenger.class.getName());
    ...
    /**
     * true if we have deliberately stopped sending.
     */
     ...
     * they require calling more event methods.
     */
    ...
    private class AsyncChannelMessengerState extends MessengerState {
        ...
        protected void failAllAction() {
            ...
            // The queue is now closed, so we can rest assured that the last message is really the last one.
    ...
    // The representation of a queued message. It is shared between this abstract class and any implementation.
     ...
     * @param redirection      Group to which the messages must be redirected. This is used to implement the automatic group
    ...
    public AsyncChannelMessenger(EndpointAddress baseAddress, PeerGroupID redirection,
        ...
        // We synchronize our state with the sharedMessenger's stateMachine. Logic would dictate that we pass it to super(),
        // but it is not itself constructed until super() returns. No way around it.
    ...
    /**
     * This internal method does the common part of sending the message on behalf of both sendMessageN abd sendMessageB.
     * <p/>
     * <p/>It is not quite possible to implement sendMessageB as a wrapper around sendMessageN without some internal
     * cooperation. At least not in an efficient manner. sendMessageB must not set the message property: either it fails and
     * throws, or it returns succesfully and the property is set later. This is required so that messages can be retried when
     * failing synchronously (through a blocking messenger typically, but the semantic has to be uniform).
     * <p/>
     * <p/>Each of sendMessageB and sendMessageN takes care of status reporting on its own terms.
     *
     * @return The outcome from that one attempt. True means done. False means saturated.  When true is returned, it means that
     *         the fate of the message will be decided asynchronously, so we do not have any details, yet.
     * @throws IOException if this messenger is closed.
     */
    private final boolean sendMessageCommon(Message msg, String rService, String rServiceParam) throws IOException {
        ...
        synchronized (stateMachine) {
            ...
            if (inq < qsz - 1) {
                ...
                // Was already saturated. Just make sure we report it. Can't hurt.
                queued = false;
            }
        }

        if (queued && change) {
            // If not queued, there was no change of condition as far as outsiders are concerned.
            // (redundant saturatedEvent, only defensive; to guarantee statemachine in sync).
            // else, if the saturation state did not change, we have no state change to notify.
            notifyChange();
        }

        performDeferredAction(action);

        // Before we return, make sure that this channel remains referenced if it has messages.
        // It could become unreferenced if it is not yet resolved and the application lets go
        // of it after sending messages. This means that we may need to do something only
        // in the resolpending and resolsaturated cases. The way we do this test, there can
        // be false positives. They're dealt with as part of the action that is carried out.
    ...
    public final void sendMessageB(Message msg, String rService, String rServiceParam) throws IOException {
        ...
        try {
            while (true) {
                ...
                // Do a shallow check on the queue. If it seems empty (without getting into a critical section to verify it), then
                // yielding is good bet. It is a lot cheaper and smoother than waiting.
     ...
     * <p/>
     * DEAD LOCK WARNING: the implementor's method invoke some of our call backs while synchronized. Then our call backs synchronize
     * on the state machine in here. This nesting order must always be respected. As a result, we can never invoke
     * implementors methods while synchronized. Hence the deferredAction processing.
     */
    ...
    /**
     * Here, we behave like a queue to the shared messenger. When we report being empty, though, we're automatically removed from
     * the active queues list. We'll go back there the next time we have something to send by calling startImpl.
     */
    protected PendingMessage peek() {
        ...
        synchronized (stateMachine) {
            ...
            // We like the msg to keep occupying space in the queue until it's out the door.
            // That way, idleness (that is, not currently working on a message), is always
            // consistent with queue emptyness.
            ...
            if (theMsg == null) {
                ...
                // We do not notifyChange, here, because, if the queue is empty, it was already notified when the last message was
                // poped. The call to idleEvent is only defensive programming to make extra sure the state machine is in sync.

                return null;
            }

            if (outputClosed) {
                // We've been asked to stop sending. Which, if we were sending, must be notified by either an idle event or a down
                // event. Nothing needs to happen to the shared messenger. We're just a channel.
                stateMachine.downEvent();
                action = eventCalled(true);
                theMsg = null;
            }
     ...
     *
     * @return true if there are more messages after the one we poped.
     */
     ...
     * <p/>
     * NOTE that it is the shared messenger responsability to synchronize so that we cannot
     * be added to the active list just before we get removed due to reporting an empty queue in parallel.
     * So, if we report an empty queue and have a new message to send before the shared messenger removes
     * us form the active list, startImpl will block until the removal is done. Then we'll be added back.
     * <p/>
     * If it cannot be done, it means that the shared messenger is no longer usable. It may call down() in sequence.
     * Out of defensiveness, it should do so without holding its lock.
     */
     ...
     * <p/>
     * If it cannot be done, it means that the shared messenger is no longer usable. It may call down() in sequence.
     * Out of defensiveness, it should do so without holding its lock. If the messenger is already resolved
     * it may call up() in sequence. Same wisdom applies. It is a good idea to create channels in the resolved
     * state if the shared messenger is already resolved. That avoids this extra contortion.
     */
     ...
     * resolSaturated state. This is specific to this type of channels. The shared messenger must
     * make sure that this channel remains strongly referenced, eventhough it is not resolved,
     * because there are messages in it. It is valid for an application to let go of a channel
     * after sending a message, even if the channel is not yet resolved. The message will go
     * if/when the channel resolves. This method may be invoked redundantly and even once
     * the channel is no longer among the one awaiting resolution. The implementation
     * must be carefull to ignore such calls.
     */
